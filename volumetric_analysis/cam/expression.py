...
class Matrix:
    ...
    def load_genes(self,splice=False):
        """
        Load genes from self.fgenes
        """
        gene_stats = aux.read.into_list2(self.fgenes)
        self.genes = {}
        self.gene_idx = {}
        idx = 0
        for g in gene_stats:
            ...
            if splice:
                _idx = []
                for j in range(isoforms):
                    _idx.append(idx)
                    self.gene_idx[idx] = name
                    idx += 1
                self.genes[name] = Gene(_idx,name,isoforms)
            else:
                self.genes[name] = Gene(idx,name,isoforms)
                self.gene_idx[idx] = name
                idx += 1
        self.n = idx
    ...
    def load_cells(self,cells):
        self.m = len(cells)
        self.cells = dict([(cells[i],i) for i in range(self.m)])
        self.cells_idx = {y:x for x,y in self.cells.items()}

    def assign_expression(self):
        if not self.n: raise('Error: must first load genes: Matrix.load_genes')
        if not self.m: raise('Error: must firs load cells: Matrix.load_cells')
        
        data = aux.read.into_list2(self.fexp)
        self.M = np.zeros((self.m,self.n))

        for [g,c,val] in data:
            if g not in self.genes: continue
            if c not in self.cells: continue
            idx = self.cells[c]
...
class Gene:
    ...
    """  
    def __init__(self,idx,gene,isoforms):
        self.idx = idx
        self.gene = gene
